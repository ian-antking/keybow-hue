...
def build_increase_hue_button(room, keyboard, key_index):
    return keyboard.Key(key_index, room.increase_hue, lambda: colorsys.hsv_to_rgb(360 * (int((room.get_state('hue') + 6553) / 65535)), int((room.get_state('sat') / 254) * 100), int((room.get_state('bri') / 254) * 100)))

def build_decrease_hue_button(room, keyboard, key_index):
    return keyboard.Key(key_index, room.decrease_hue, lambda: colorsys.hsv_to_rgb(360 * (int((room.get_state('hue') - 6553) / 65535)), int((room.get_state('sat') / 254) * 100), int((room.get_state('bri') / 254) * 100)))